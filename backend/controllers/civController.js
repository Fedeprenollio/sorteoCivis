...
export const crearJugador = async (req, res) => {
  ...
  if (!jugador) return res.status(400).json({ error: "Falta el nombre del jugador." });
...
export const asignarCivJugador = async (req, res) => {
  ...
  try {
    ...
    if (user.disponibles.length === 0) {
      return res.status(400).json({ error: "No quedan civilizaciones disponibles." });
    }
...
export const agregarCivManualmente = async (req, res) => {
  ...
  if (!jugador || !civ) return res.status(400).json({ error: "Falta jugador o civi." });
...
export const eliminarJugador = async (req, res) => {
  ...
  try {
    ...
    if (!eliminado) return res.status(404).json({ error: "Jugador no existe." });
...
export const eliminarCiviUsada = async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, mensaje: `Civi "${civiNombre}" eliminada de usadas.` });
  } catch (error) {
